[HOME](http://xpg.me/)               [TAOBAO](https://xpgt12.taobao.com/)                         [BUY](https://item.taobao.com/item.htm?id=1059072789792)
...
- The button highlights when heating is on, 和 returns to normal style when off
...
<body>
  <h1>GitHub Issues 模板</h1>
  <p>本模板可用于为 GitHub 仓库的 Issues 添加标准化格式，提升协作效率。</p>

  <div class="section">
    <h3>📌 描述</h3>
    <p>请清晰描述你遇到的问题、建议或功能需求。</p>
    <ul>
      <li>发生了什么？（预期 vs 实际）</li>
      <li>如何复现？请提供步骤</li>
      <li>是否有相关截图或日志？</li>
    </ul>
  </div>
  ...
  <div class="section">
    <h3>🛠️ 环境信息</h3>
    <ul>
      <li>操作系统：<code>macOS 14 / Windows 11 / Ubuntu 22.04</code></li>
      <li>浏览器/终端：<code>Chrome 120 / Firefox 121 / Terminal</code></li>
      <li>版本：<code>v1.2.3</code> 或 <code>commit: a1b2c3d</code></li>
    </ul>
  </div>

  <div class="section">
    <h3>✅ 预期行为</h3>
    <p>例如：<em>点击按钮后应弹出确认框，但实际未触发。</em></p>
  </div>

  <div class="section">
    <h3>❌ 实际行为</h3>
    <p>例如：<em>点击按钮无反应，控制台报错：Uncaught TypeError: Cannot read property 'xxx' of undefined</em></p>
  </div>

  <div class="section">
    <h3>📝 复现步骤</h3>
    <ol>
      <li>进入页面 <code>/dashboard</code></li>
      <li>点击“导出”按钮</li>
      <li>观察是否弹出下载对话框</li>
    </ol>
  </div>
  ...
  <div class="section">
    <h3>🧩 可能的解决方案（可选）</h3>
    <p>如果你有想法，欢迎简要说明修复思路。</p>
  </div>

  <div class="note">
    <strong>💡 提示：</strong> 提交前请检查是否已有类似 Issue，避免重复。对于 bug 报告，请提供最小复现仓库链接或在线演示（如 CodeSandbox）。
  </div>

  <h2>如何在 GitHub 仓库中启用此模板？</h2>
  <ol>
    <li>在仓库根目录创建 <code>.github/ISSUE_TEMPLATE/config.yml</code>（可选）配置模板分类</li>
    <li>添加 <code>.github/ISSUE_TEMPLATE/bug_report.md</code>、<code>feature_request.md</code> 等文件</li>
    <li>粘贴上述内容到对应模板文件中</li>
    <li>提交并推送到 GitHub</li>
  </ol>
  <p>完成后，新建 Issue 时将自动展示模板选项。</p>
</body>
</html>
